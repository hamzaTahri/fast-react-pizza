    ...
    return (
        <div className="space-y-8 px-4 py-6">
            ...
            <ul className="dive-stone-200 divide-y border-b border-t">
                {cart.map((item) => (
                    <OrderItem item={item} isLoadingIngredients={fetcher?.state ==='loading'} ingredients={fetcher?.data?.find(element=> element.id === item.pizzaId).ingredients??[]} key={item.pizzaId}/>
                ))}
            </ul>
